...
int dpdk_ext_tx_worker(void *arg)
{
    ...
    while (!(*params->stop_flag))
    {
        ...
        *(uint64_t *)payload = seq;
        ...
        // PRBS data (son byte hariç) + DTN sequence number (son byte)
#if IMIX_ENABLED
        uint64_t prbs_offset = (seq * (uint64_t)MAX_PRBS_BYTES) % PRBS_CACHE_SIZE;
        memcpy(payload + 8, prbs_cache_ext + prbs_offset, prbs_len - 1);
        payload[8 + prbs_len - 1] = calc_dtn_seq(seq);
#else
        uint64_t prbs_offset = (seq * (uint64_t)NUM_PRBS_BYTES) % PRBS_CACHE_SIZE;
        memcpy(payload + 8, prbs_cache_ext + prbs_offset, NUM_PRBS_BYTES - 1);
        payload[8 + NUM_PRBS_BYTES - 1] = calc_dtn_seq(seq);
#endif
