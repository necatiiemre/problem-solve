                     ...
                     uint16_t vl_id, uint64_t sequence, const uint8_t *prbs_data)
{
    ...
    memcpy(payload + RAW_PKT_SEQ_BYTES, prbs_data, RAW_PKT_PRBS_BYTES - 1);
    // Son byte: DTN paket sequence number
    payload[RAW_PKT_SEQ_BYTES + RAW_PKT_PRBS_BYTES - 1] = calc_dtn_seq(sequence);
                              ...
                              uint16_t pkt_size)
{
    ...
    // Payload (dinamik PRBS boyutu, son byte DTN seq)
    uint8_t *payload = udp + RAW_PKT_UDP_HDR_SIZE;
    memcpy(payload, &sequence, RAW_PKT_SEQ_BYTES);
    memcpy(payload + RAW_PKT_SEQ_BYTES, prbs_data, prbs_len - 1);
    // Son byte: DTN paket sequence number
    payload[RAW_PKT_SEQ_BYTES + prbs_len - 1] = calc_dtn_seq(sequence);
    ...
    while (!port->stop_flag && (g_stop_flag == NULL || !*g_stop_flag)) {
        ...
        if (dpdk_src_port >= 0) {
            ...
            if (dpdk_prbs_cache) {
                ...
                if (cmp_bytes > 0) cmp_bytes--;  // Son byte DTN seq, PRBS değil
        ...
        if (partner && partner->prbs_initialized) {
            ...
            uint16_t prbs_cmp_len = (prbs_len > 0) ? prbs_len - 1 : 0;

            uint64_t prbs_offset = (seq * (uint64_t)RAW_MAX_PRBS_BYTES) % RAW_PRBS_CACHE_SIZE;
            uint8_t *expected_prbs = partner->prbs_cache_ext + prbs_offset;

            if (memcmp(recv_prbs, expected_prbs, prbs_cmp_len) == 0) {
                pthread_spin_lock(&source->stats.lock);
                ...
                for (uint16_t i = 0; i < prbs_cmp_len; i++) {
            ...
            uint8_t *expected_prbs = partner->prbs_cache_ext + prbs_offset;
            // Son byte DTN seq olduğu için PRBS kontrolünden 1 byte düş
            uint16_t raw_cmp_len = RAW_PKT_PRBS_BYTES - 1;

            if (memcmp(recv_prbs, expected_prbs, raw_cmp_len) == 0) {
                pthread_spin_lock(&source->stats.lock);
                source->stats.good_pkts++;
                pthread_spin_unlock(&source->stats.lock);
            } else {
