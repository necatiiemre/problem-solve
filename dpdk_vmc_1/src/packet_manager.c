                                       ...
                                       uint16_t prbs_len)
{
    ...
    *seq_ptr = sequence_number;
    ...
    // Use extended cache - son byte hariç PRBS kopyala (son byte DTN seq olacak)
    rte_memcpy(prbs_ptr,
               &port_prbs_cache[port_id].cache_ext[start_offset],
               prbs_len - 1);

    // Son byte: DTN paket sequence number (per VL-IDX, 0→255→1→255→...)
    prbs_ptr[prbs_len - 1] = calc_dtn_seq(sequence_number);
}
...
void init_packet_config(struct packet_config *config)
{
    ...
    config->src_mac.addr_bytes[5] = 0x40;
